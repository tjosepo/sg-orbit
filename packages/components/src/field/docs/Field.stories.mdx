...
import { Field, Form, GroupField, HelpMessage, InnerErrorMessage, InnerField, InnerGroupField, InnerHelpMessage, InnerLabel, InnerValidMessage, Label } from "@components/field";
...
A field can have a required state. This will add the appropriate aria-required on the main field element(input/select). You are responsible of validating the input as well as adding a required indicator if necessary.
...
A field can have a necessity indicator as a suffix on the label.

<Preview>
    <Story name="necessity indicator">
        <Form>
            <Field required>
                <Label necessityIndicator="required">Space ID</Label>
                <TextInput placeholder="999-999-999" />
                <HelpMessage>Your space id is a 9-digit number available at the top right corner of your membership card.</HelpMessage>
            </Field>
            <Field required>
                <Label necessityIndicator="optional">Suit color</Label>
                <TextInput placeholder="Red" />
            </Field>
        </Form>>
    </Story>
</Preview>
